...
const DAYS_BASE = [
  {
    ...
    choices: [
      { label: 'У входа в парк — много прохожих, аренда 150 руб. в день', delta: -150, popularityDelta: 12, comment: 'Место бойкое, народ замечает киоск. Популярность растёт.', hint: 'Высокая аренда часто окупается большим потоком.', outro: 'Аренда — постоянный расход. Умение оценить, окупится ли он выручкой, отличает успешный бизнес.' },
      { label: 'Подальше, у скамеек — аренда 40 руб., людей меньше', delta: -40, popularityDelta: 2, comment: 'Тише, но и выручка скромнее. Зато экономнее.', hint: 'Дешевле — не всегда выгоднее в итоге.', outro: 'Низкие расходы не всегда дают большую прибыль: важно, сколько людей готовы купить.' },
    ],
  },
  {
    day: 2,
  ...
  },
  {
    ...
    choices: [
      ...
      { label: 'Акция "Горячий чай с лимоном" — доп. затраты 80 руб.', delta: -80, popularityDelta: 14, comment: 'В дождь чай разошёлся на ура! Выручка +180.', hint: 'Адаптация к обстоятельствам приносит результат.', outro: 'Гибкость — навык предпринимателя.' },
    ],
  },
  {
    day: 4,
  ...
  },
  {
    ...
    choices: [
      ...
      { label: 'Уникальное предложение — наклейки к стакану (40 руб.)', delta: -40, popularityDelta: 18, comment: 'Наклейки понравились! Ты выделился.', hint: 'Уникальность важнее гонки цен.', outro: 'Уникальное предложение выделяет среди конкурентов.' },
  ...
  },
  {
    ...
    choices: [
      ...
      { label: 'Дать взятку 250 руб. (риск)', delta: -250, popularityDelta: -12, comment: 'Рискованно. Лучше так не делать.', hint: 'Взятка — преступление и риск.', outro: 'Легальное ведение бизнеса надёжнее.' },
  ...
  },
  {
    ...
    text: 'Бабушка предлагает свой секретный рецепт лимонада за 80 руб.',
    choices: [
  ...
  },
  {
    ...
    choices: [
      { label: 'Закупить на 180 руб. и продавать активно', delta: -180, popularityDelta: 8, comment: 'Продал почти всё. Выручка +420. Отличный день!', hint: 'Умение оценить спрос приносит прибыль.', outro: 'Готовность рискнуть в удачный день приносит прибыль.' },
  ...
  },
  {
    ...
    choices: [
      { label: 'Срочно починить — 120 руб.', delta: -120, popularityDelta: 0, comment: 'Починил. Работа идёт дальше.', hint: 'Техника — часть расходов бизнеса.', outro: 'Резервный фонд помогает на ремонт.' },
  ...
  },
  {
    ...
    text: 'Друг готов помогать за 70 руб. в день. С ним успеешь обслужить больше клиентов.',
    choices: [
  ...
  },
  {
    ...
    text: 'Можно заплатить 50 руб. за рекламный пост в местной группе парка.',
    choices: [
  ...
  },
  {
    ...
    choices: [
      { label: 'Сделать вторую стойку — доп. 80 руб.', delta: -80, popularityDelta: 12, comment: 'Очередь раздвоилась. Все довольны. Продажи выросли.', hint: 'Удобство клиентов = лояльность.', outro: 'Инвестиция в сервис окупается.' },
...
const BUSINESS_RANDOM_EVENTS = [
  ...
  },
  {
    ...
    choices: [
      { label: 'Купить новый зонт — 90 руб.', delta: -90, popularityDelta: 6, comment: 'Тень вернулась. Очередь снова растёт.', hint: 'Удобство клиентов.', outro: 'Мелкие вложения в комфорт окупаются.' },
  ...
  },
  {
    ...
    text: 'Представитель сети кафе предлагает купить твой рецепт за 400 руб. разово.',
    choices: [
...
const DAYS_PLOT_16_30 = [
  ...
  },
  {
    ...
    text: 'Родители видят, что ты устаёшь. Готовы дать 150 руб. на помощника на неделю.',
    choices: [
  ...
  },
  {
    ...
    text: 'Осталась последняя неделя. Цель — 5000 руб. — уже близко. Нужен последний рывок.',
    choices: [
...
export default function ScenarioBusiness({
  ...
  useEffect(() => {
    ...
    async function initRun() {
      try {
        ...
        setBudget(Math.round(Number.isFinite(run.budget) ? run.budget : startCapital));
  ...
  const finishScenario = async (finalBudget) => {
    ...
    let text =
      passed
        ? `Ты заработал(а) ${finalBudget} руб.! Цель (${goal} на смартфон) достигнута. Прибыль за каникулы: +${profit} руб. Ты узнал(а), что бизнес — это вложения, риск и награда.`
        : finalBudget >= startCapital
        ? `Итог: ${finalBudget} руб. Прибыль +${finalBudget - startCapital}, но до смартфона не хватило. Ты приобрёл(а) опыт: аренда, конкуренция, маркетинг — всё это часть предпринимательства.`
        : `Итог: ${finalBudget} руб. Убыток ${startCapital - finalBudget}. Бизнес — это риск: не всегда получается с первого раза. Зато ты узнал(а) про себестоимость, спрос и конкуренцию.`;
    setFinished(true);
    setResultText(text);
    setPhase('result');
  ...
  const handleChoice = (choice) => {
    ...
    const dailyLine = `\n\nДневная выручка киоска: +${dailyIncome} руб.`;
  ...
  const exitScenario = async () => {
    try {
      ...
      // still go back on network error
    }
    onBackToMap();
  };
      ...
      <ScenarioStage
        ...
        leftHud={
          <>
            ...
            <div className="hud-pill"><span className="hud-label">💰</span><span className="hud-value">{Math.round(budget)}</span></div>
          </>
        }
        onExit={exitScenario}
      >
  ...
  const leftHud = (
    <>
      ...
      <div className="hud-pill">
        ...
        <span className="hud-value">{Math.round(budget)}</span>
      </div>
